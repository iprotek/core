...
window.WebRequest2 = function(_method, _url, _content = null, _contentType ='application/json'  ){
  ...
  var request  = null;var xcsrftoken = document.querySelector('meta[name="csrf-token"]');
  if(xcsrftoken)
  {
    xcsrftoken = xcsrftoken.getAttribute('content');
  }
  ...
  if(_contentType == "multipart/form-data"){
    ...
    request =  _content ? ParseObjectToString(_contentType, _content) : ( _method.toUpperCase() == 'POST' ? '{}' :  null);
    headers = {
  ...
  return fetch(_url, { method:_method, body: request, credentials: 'include', headers:headers});
  /*
  .then(response => {
      if (response.ok) 
